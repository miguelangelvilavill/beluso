...
document.addEventListener('DOMContentLoaded', () => {
  // --- 0. Lgica de Pantalla de Inicio (Splash Screen) ---
  ...
  // --- 2.0 Lgica de Geolocalización del Usuario ---
  ...
  */
  ...
  // Lgica de pestañas para el modal de planes
  ...
  const fetchRealWeather = async () => {
    try {
      ...
      if (mIcon) {
        let icon = '☀';
        if (state === 'rain') icon = '🌧';
        else if (state === 'thunder') icon = '⛈';
        else if (state === 'cloudy') icon = '🌥';
      ...
      if (hourlyList) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          let hIcon = '☀';
          if (hSym.includes('rain')) hIcon = '🌧';
          else if (hSym.includes('cloud')) hIcon = '🌥';
      ...
      if (dailyList) {
        ...
        days.forEach((dateKey) => {
          ...
          let dIcon = '☀';
          if (dSym.includes('rain')) dIcon = '🌧';
          else if (dSym.includes('cloud')) dIcon = '🌥';
          dHtml += `
  ...
  // --- 9. Lgica de Interacción con el Espíritu del Tiempo ---
  let moodClicks = 0;
